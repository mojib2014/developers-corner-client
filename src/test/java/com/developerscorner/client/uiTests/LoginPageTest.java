...
import java.time.Duration;import org.openqa.selenium.support.PageFactory;
...
public class LoginPageTest extends SeleniumConfig {
	...
	private String homePageUrl = "http://localhost:8080/#!/";
	...
	public void shouldLogInAUser() {
		try {
			...
			assertTrue(form.loginBtn.isEnabled(), "Login button should be enabled at this point");
			
			form.submit();
			new WebDriverWait(driver, Duration.ofSeconds(5)).until(ExpectedConditions.urlToBe(homePageUrl));
			assertEquals(driver.getCurrentUrl(), homePageUrl, "Should be redirected to the home page at this point");
		}catch(Exception e) {
			e.printStackTrace();
		}
	}
	...
	void shouldDisplayFieldRequiredIfEmailNotFilled() {
		...
		assertFalse(form.loginBtn.isEnabled(), "Login button should be disabled at this point");
